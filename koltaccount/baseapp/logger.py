...
def get_logs() -> list:
    """Возвращает логи"""
    logs = read_log_file(BASE_DIR / "z_logs/log.json")
    if logs != "{}":
...
def write_error_to_log_file(
    ...
    try:
        ...
        _log_file = open(BASE_DIR / "z_logs/log.json")
        log_file = _log_file.read()
        _log_file.close()
        text = json.loads(log_file)

        with open(BASE_DIR / "z_logs/log.json", "w+") as f:
            text.update(
                {
                    len(text): {
                        "type": error_type,
        ...
        if len(log_file) >= 10485760:
            with zipfile.ZipFile(
                BASE_DIR / "z_logs/log_json__"
                + datetime.now().strftime("%d-%m-%Y_%H-%M-%S")
                + "__.zip",
                "w",
            ) as arzip:
                arzip.write(BASE_DIR / "z_logs/log.json")
                log_file = open(BASE_DIR / "z_logs/log.json", "w+")
                log_file.write(json.dumps(json.loads("{}"), indent=4))
                log_file.close()
    except FileNotFoundError:
        if not os.path.exists(BASE_DIR / "z_logs"):
            os.mkdir(BASE_DIR / "z_logs")
